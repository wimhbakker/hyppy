...
class Application(Frame):
    ...
    def do_run(self) :
        ...
        try:
            spectralbinning.spectral_binning(self.nameIn.get(), self.nameOut.get(),
                  binsize=self.binSize.get(),
...
##            self.nameOffset.set(name)
    ...
    def makeWindow(self):
        ...
        self.binSize = IntVar()
        self.binSize.set(conf.get_option('bin-size', 3, type_=int))
        ...
        Label(frame, text="Z Bin Size (odd): ").grid(row=0, column=0, sticky=W)
        Entry(frame, textvariable=self.binSize, width=5).grid(row=0, column=1, sticky=W)
...
conf.set_option('bin-size', app.binSize.get())
