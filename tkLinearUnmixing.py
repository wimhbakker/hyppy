...
class Application(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.message("""Use normalize:False for reflectance (0-1).
Use normalize:True for all other input.""")
    
    def do_run(self) :
        self.message("In: " + self.nameIn.get())
        self.message("Out: " + self.nameOut.get())
        self.message("Normalize: " + str(self.normalize.get()))

        self.message("Generating rule images...")
        try:
            linearunmixing.linear_unmixing(self.nameIn.get(), self.nameOut.get(),
...
#            self.message('Exception: %s' % (str(err),))
            self.message('Traceback: %s' % (traceback.format_exc(),))
...
##        else:
    ...
    def makeWindow(self):
        ...
        self.normalize.set(conf.get_option('normalize', True, type_=bool))
        ...
        Label(self.frame0, text="Normalize image:").grid(row=0, column=0, sticky=E)
        Radiobutton(self.frame0, variable=self.normalize, value=True, text='True').grid(row=0, column=1, sticky=W)
        Radiobutton(self.frame0, variable=self.normalize, value=False, text='False').grid(row=1, column=1, sticky=W)
        self.normalize.set(conf.get_option('normalize', True))
...
conf.set_option('normalize', app.normalize.get())
