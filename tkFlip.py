...
#
#     tkFlip.py
##  Created 20180912 WHB determine smile
##
...
class Application(Frame):
    ...
    def do_run(self) :
        ...
        self.message("Flipping, please wait...")
        try:
            flip.flip(self.nameIn.get(), self.nameOut.get(),
                      mode=self.flipMode.get(),
    ...
    def makeWindow(self):
        ...
        # frame4 with flipmode
        frame4 = Frame(self, bd=1, relief=GROOVE)
        frame4.grid(row=row, column=0, columnspan=3, sticky=W+E)
        frame4.columnconfigure(0, weight=1)
        frame4.columnconfigure(1, weight=1)
        frame4.columnconfigure(2, weight=1)

        Radiobutton(frame4, text='flip x', value='x', variable=self.flipMode).grid(row=0, column=0, sticky=W+E)
        Radiobutton(frame4, text='flip y', value='y', variable=self.flipMode).grid(row=0, column=1, sticky=W+E)
        Radiobutton(frame4, text='flip z', value='z', variable=self.flipMode).grid(row=0, column=2, sticky=W+E)
        self.flipMode.set(conf.get_option('flipmode', 'x'))
...
conf.set_option('flipmode', app.flipMode.get())
