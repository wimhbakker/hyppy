...
class Application(Frame):
    ...
    def do_run(self) :
        ...
        try:
            banddepths.banddepths(self.nameIn.get(), self.nameOut.get(), delta=self.delta.get(),
                      sort_wavelengths=self.sortWav.get(),
                      use_bbl=self.useBBL.get(), progress=self.progressBar)
...
#            if self.nameOut.get() == "":
            if name[-4] != '.':
                self.nameOut.set(name + POSTFIX + str(self.delta.get()))
            else:
                names = name.rsplit('.', 1)
                self.nameOut.set(names[0] + POSTFIX + str(self.delta.get()) + '.' + names[1])
    ...
    def makeWindow(self):
        # variables
        self.delta = IntVar()
        self.delta.set(conf.get_option('delta', 1))
        ...
        Label(self.frame0, text="Delta (band-shift between bands)").grid(row=0, column=1, sticky=W)
...
conf.set_option('delta', app.delta.get())
