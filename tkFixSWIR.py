...
#
#     tkFixSWIR.py
##  Created 201800913 WHB for fixing every eighth sample
##
...
class Application(Frame):
    ...
    def do_run(self) :
        ...
        try:
            try:
                ...
                start = None
            fixswir.fixswir(self.nameIn.get(), self.nameOut.get(),
                            start=start,
    ...
    def makeWindow(self):
        ...
        self.frame0.grid(row=row, column=0, sticky=W)

        Label(self.frame0, text="First faulty column number x=").grid(row=0, column=0, sticky=W, columnspan=5)
        Radiobutton(self.frame0, variable=self.start, value='auto', text='auto').grid(row=1, column=0, sticky=W)
        for i in range(8):
            Radiobutton(self.frame0, variable=self.start, value=str(i), text=str(i)).grid(row=1, column=i+1, sticky=W)
        self.start.set(conf.get_option('start','1'))
...
conf.set_option('start', app.start.get())
