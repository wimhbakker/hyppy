...
DESCRIPTION = "Band Normalization"
...
class Application(Frame):
    ...
    def do_run(self) :
        ...
        try:
            normalize.normalize(self.nameIn.get(), self.nameOut.get(),
                      add_stdev=self.addStdev.get(),
...
##        self.delta.set(1)
        ...
        self.addStdev = DoubleVar()
        self.addStdev.set(conf.get_option('added-standard-deviations', 0.0))
...
##        row = row + 1
        ...
        # Add # Stdev's
        Label(self, text="Add Stddev's").grid(row=row, column=0, sticky=W)
        Entry(self, textvariable=self.addStdev, width=4).grid(row=row, column=1, sticky=W)
...
conf.set_option('added-standard-deviations', app.addStdev.get())
